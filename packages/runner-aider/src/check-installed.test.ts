...
describe('checkInstalled (aider)', () => {
  beforeEach(() => {
    harness.state.next = {};
    harness.state.calls.length = 0;
  });

  it('returns { ok: true, version } when aider --version prints a semver-ish string', async () => {
    setExecResult({ stdout: 'aider 0.55.0\n' });
  ...
  it('parses version from stderr when stdout is empty', async () => {
    setExecResult({ stdout: '', stderr: 'aider 1.2.3-dev\n' });
    const result = await checkInstalled();
    expect(result.ok).toBe(true);
    expect(result.version).toBe('1.2.3-dev');
  });

  it('returns { ok: true, version: undefined } when output lacks a parseable version', async () => {
    setExecResult({ stdout: 'aider is alive\n' });
    const result = await checkInstalled();
    expect(result.ok).toBe(true);
...
describe('autoDetect (aider)', () => {
  ...
  it('returns the first non-empty line from which/where output', async () => {
    setExecResult({ stdout: '/usr/local/bin/aider\n' });
    const result = await autoDetect();
    expect(result).toBe('/usr/local/bin/aider');
  });
