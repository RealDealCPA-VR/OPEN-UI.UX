...
function fallbackCandidates(workspaceRoot?: string): string[] {
  if (process.platform === 'win32') {
    ...
    for (const ext of exts) {
      ...
      if (appData) {
        list.push(`${appData}\\npm\\${CLI_NAME}.${ext}`);
        list.push(`${appData}\\Python\\Scripts\\${CLI_NAME}.${ext}`);
        list.push(`${appData}\\pipx\\venvs\\${CLI_NAME}\\Scripts\\${CLI_NAME}.${ext}`);
      }
    }
    if (workspaceRoot) {
      list.push(`${workspaceRoot}\\.venv\\Scripts\\${CLI_NAME}.exe`);
    }
    return list;
  }
  ...
  if (home) {
    ...
    list.push(`${home}/.local/pipx/venvs/${CLI_NAME}/bin/${CLI_NAME}`);
  }
