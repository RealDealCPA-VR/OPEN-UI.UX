...
function makeHost(setting?: string): PluginHost {
  return {
    ...
    getSetting: vi.fn(async (key: string) =>
      key === 'aiderCliPath' ? (setting as never) : undefined,
    ),
    setSetting: vi.fn(),
...
describe('createAiderRunner().run()', () => {
  beforeEach(() => {
  ...
  it('never enables shell:true (command-injection guard)', async () => {
    const runner = createAiderRunner(makeHost('/bin/aider'));
  ...
  it('relays stdout lines as text_delta and ends with usage + done', async () => {
    const runner = createAiderRunner(makeHost('/bin/aider'));
    ...
    expect(events).toEqual([
      { type: 'text_delta', delta: 'first line\n' },
      { type: 'text_delta', delta: 'second line\n' },
