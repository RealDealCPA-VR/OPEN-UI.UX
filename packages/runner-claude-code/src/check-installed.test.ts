...
describe('checkInstalled', () => {
  ...
  it('returns { ok: true, version } when claude --version prints a semver-ish string', async () => {
    setExecResult({ stdout: '1.2.3 (Claude Code)\n' });
  ...
  it('parses version from stderr when stdout is empty', async () => {
    setExecResult({ stdout: '', stderr: 'claude-code v0.4.10\n' });
    const result = await checkInstalled();
    expect(result.ok).toBe(true);
    expect(result.version).toBe('0.4.10');
  });

  it('returns { ok: true, version: undefined } when output lacks a parseable version', async () => {
    setExecResult({ stdout: 'claude is alive\n' });
    const result = await checkInstalled();
    expect(result.ok).toBe(true);
  ...
  it('returns { ok: false, hint } when the CLI is missing (ENOENT)', async () => {
    ...
    expect(result.hint).toBe(CLAUDE_INSTALL_HINT);
    expect(result.version).toBeUndefined();
  });

  it('uses cliPath override when provided', async () => {
    setExecResult({ stdout: '2.0.0\n' });
    await checkInstalled('/custom/path/claude');
    expect(callRecords.length).toBeGreaterThan(0);
    expect(callRecords[0]?.cmd).toBe('/custom/path/claude');
...
describe('autoDetect', () => {
  ...
  it('returns the first non-empty line from which/where output', async () => {
    setExecResult({ stdout: '/usr/local/bin/claude\n' });
