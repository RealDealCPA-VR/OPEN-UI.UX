...
vi.mock('@opencodex/core/process/tree-kill', () => ({
  treeKill: (child: { killed: boolean; exitCode: number | null }) => {
    child.killed = true;
...
function makeHost(setting?: string): PluginHost {
  return {
    ...
    getSetting: vi.fn(async (key: string) =>
      key === 'opencodeCliPath' ? (setting as never) : undefined,
    ),
    setSetting: vi.fn(),
...
async function collect(
  ...
  // Kick off the generator so the child is spawned and listeners attached.
  const first = it.next();
  // Flush microtasks until the generator has spawned the child and attached
  // its event listeners (resolveCliPath -> getSetting are async).
...
describe('createOpenCodeRunner().run()', () => {
  beforeEach(() => {
  ...
  it('never enables shell:true (command-injection guard)', async () => {
    const runner = createOpenCodeRunner(makeHost('/bin/opencode'));
  ...
  it('translates NDJSON stdout into the expected ChatEvent sequence', async () => {
    ...
    const events = await collect(runner.run(baseOpts()), (child) => {
      child.stdout.emit('data', '{"type":"text","text":"hi"}\n');
      child.stdout.emit('data', '{"type":"done","usage":{"input_tokens":3,"output_tokens":5}}\n');
      child.emit('close', 0);
    });
