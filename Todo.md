...
- [x] **Enforce the fan-out consent gate** — _shipped 2026-06-10 (audit-fix session): per-spawn gate, decision = option (a) variant with zero core changes — `apps/desktop/src/main/agent/fanout-gate.ts` keys per-run consent state on the run's AbortSignal identity (WeakMap; the chat runner threads one signal instance into every tool call of a stream). `spawn_subagent` calls `fanoutGate.ensureConsent()`: the first spawn in a parent run prompts via `requestFanoutConsent()` (threshold = 1), approved runs don't re-prompt, denial fails the tool call with a model-readable error and resets so a revised spawn re-prompts. Renderer modal round-trip needs app-run verification._
- [x] **Dispatch plugin slash commands** — _shipped 2026-06-10 (audit-fix session): `plugins:list-slash-commands` + `plugins:run-slash-command` IPC (Zod-validated), `apps/desktop/src/main/plugins/slash-dispatch.ts` dispatcher gated on plugin status `loaded`, composer slash menu appends "Plugin — <displayName>" groups after Skills/MCP, and invocation mirrors `/skill:` — send-time `detectPluginCommandInvocation` in `slash-commands.ts` dispatches and appends the result as a system message. Name collisions across plugins resolve to first registration; namespaced inserts are a possible follow-up._
