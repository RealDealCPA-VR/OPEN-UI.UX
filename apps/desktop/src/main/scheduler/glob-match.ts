/**
 * Glob → RegExp converter used by the file-change trigger watcher. Mirrors
 * `packages/tools/src/glob-match.ts` (which is consumed by the agent's `glob`
 * tool). Duplicating it here avoids cross-package imports from main-process
 * code into the agent tool layer for what is otherwise an internal helper.
 *
 * Supports:
 *   *          → any non-`/` segment
 ...
 *   {a,b,c}    → top-level brace expansion (recursive)
 */
...
/** Convenience wrapper — returns true when `path` matches `glob`. */
